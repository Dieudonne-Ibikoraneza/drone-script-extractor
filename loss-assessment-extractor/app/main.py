"""
FastAPI application for loss assessment PDF extraction service.
"""
...
from app.extractor import LossAssessmentExtractor
...
# Create FastAPI app
app = FastAPI(
    title="Loss Assessment PDF Extraction Service",
    description="Microservice for extracting structured data from loss assessment PDF reports",
    version="1.0.0"
)
...
@app.post("/extract-loss-assessment-data", response_model=ExtractResponse)
async def extract_loss_assessment_data(request: ExtractRequest) -> ExtractResponse:
    """
    Extract structured data from a loss assessment PDF report.
    ...
    """
    logger.info("=" * 50)
    logger.info("Received POST request to /extract-loss-assessment-data")
    ...
    try:
        ...
        try:
            ...
            extractor = LossAssessmentExtractor(pdf_path)
...
if __name__ == "__main__":
    import uvicorn
    logger.info("=" * 50)
    logger.info("Starting Loss Assessment PDF Extraction Service")
